...
Review the **Residual Flags - REVIEW HERE** sheet after each run - not the **Residual Flags - Full Detail** sheet, which is per-occurrence detail for reference only (see below). Residual items are candidates for manual review, not automatic redactions of every capitalised word. No detection approach reaches 100% recall on free text - industry benchmarks put general-purpose PII tools at 57-73% recall on real enterprise data, so the review step is load-bearing, not a formality.

### Residual Flags Summary (making review actually feasible at scale)
The same flagged value routinely recurs hundreds or thousands of times across a large file (e.g. a missed surname appearing in many rows) - at 100k rows, a per-occurrence detail sheet can have tens of thousands of rows, which nobody reviews row by row in practice. The output workbook's **Residual Flags - REVIEW HERE** sheet groups by unique (text, label) combination instead, most-frequent first, with an occurrence count and a context snippet. Measured on a 12k-row test file: 2,785 raw residual rows collapsed to 14 unique lines to actually read - a ~200x reduction, with the full per-row detail still available in **Residual Flags - Full Detail** for anyone who wants to check exact locations. The two sheets are named and labelled explicitly (including an instruction row inside each) after a real case where a reviewer opened the 557-row detail sheet first and assumed that was what needed manual checking, instead of the 16-line summary covering the same findings.
...
- `Residual Flags - REVIEW HERE` (if present): one line per unique flagged value with an occurrence count and context snippet - this is the sheet to manually review, not the detail one below
- `Residual Flags - Full Detail` (if present): every individual occurrence, for reference/traceability only - not meant for row-by-row manual review
